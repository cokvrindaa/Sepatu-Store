<div class="w-full max-w-2xl space-y-6">
    ...
    <form wire:submit.prevent="submit" class="bg-white rounded-3xl shadow border p-6 space-y-4" id="booking-form">
      ...
      @if (session()->has('error'))
        <p class="text-sm font-semibold" style="color: red;">{{ session('error') }}</p>
      @elseif (session()->has('message'))
        <p class="text-sm font-semibold" style="color: green;">{{ session('message') }}</p>
      @endif
      ...
      <div class="flex justify-between"><span>Discount</span><span id="discount-value" class="font-bold ">- Rp {{ number_format($discount) }}</span></div>
    </form>
    <div class="fixed bottom-4 left-4 right-4 z-50 md:static">
      <div class="flex items-center justify-between rounded-full bg-[#2A2A2A] p-2.5 pl-6 shadow-2xl">
        ...
        <button  form="booking-form" class="rounded-full px-6 py-3 bg-[#C5F277] font-extrabold text-sm text-black">
          Continue
        </button>
      </div>
    </div>
